...
type BitReader<'a> = bitstream_io::AsyncBitReader<&'a mut dyn io::Read, bitstream_io::BigEndian>;
...
pub async fn read_int<R: AsyncRead + Send + Sync + Unpin>(r: &mut R, size: u64) -> MResult<i64> {
    let mut r = AsyncBitReader::<_, BigEndian>::new(r);
    match size {
        0 => Ok(0),
        s @ 1..=8 => r.read_signed(s as u32 * 8).await.map_err(MatroskaError::Io),
        _ => Err(MatroskaError::InvalidUint),
    }
}

pub async fn read_uint<R: AsyncRead + Send + Sync + Unpin>(r: &mut R, size: u64) -> MResult<u64> {
    let mut r = AsyncBitReader::<_, BigEndian>::new(r);
    match size {
        0 => Ok(0),
        s @ 1..=8 => r.read(s as u32 * 8).await.map_err(MatroskaError::Io),
        _ => Err(MatroskaError::InvalidUint),
    }
}

pub async fn read_float<R: AsyncRead + Send + Sync + Unpin>(r: &mut R, size: u64) -> MResult<f64> {
    let mut r = AsyncBitReader::<_, BigEndian>::new(r);
    match size {
        4 => {
            let i: u32 = r.read(32).await.map_err(MatroskaError::Io)?;
            let f = f32::from_bits(i);
            Ok(f64::from(f))
        }
        8 => {
            let i: u64 = r.read(64).await.map_err(MatroskaError::Io)?;
