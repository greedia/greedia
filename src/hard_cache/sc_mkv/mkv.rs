...
impl Matroska {
    ...
    pub async fn open<R: AsyncRead + AsyncSeek + Send + Sync + Unpin>(
        ...
        let (mut id_0, mut size_0, _) = ebml::read_element_id_size(&mut file).await?;
        let m = Element::parse_master(&mut file, size_0).await.unwrap();
        if id_0 == ids::EBML_HEADER {
            for e in m {
                if e.id == ids::EBML_DOC_TYPE {
            ...
            match id_1 {
                ids::SEEKHEAD => {
                    ...
                    if let Some(pos) = seektable.get(ids::CHAPTERS) {
                        ...
                        matroska.chapters = ChapterEdition::parse(&mut file, s).await?;
                    }
                    return Ok(matroska);
                }
                // if no seektable, populate file from parts
                ids::INFO => {
                    matroska.info = Info::parse(&mut file, size_1).await?;
                }
                ids::TRACKS => {
                    matroska.tracks = Track::parse(&mut file, size_1).await?;
                }
                ids::ATTACHMENTS => {
                    matroska.attachments = Attachment::parse(&mut file, size_1).await?;
                }
                ids::CHAPTERS => {
...
impl Seek {
    ...
    pub(super) fn build(elements: Vec<Element>) -> Seek {
        ...
        for e in Element::parse_master(r, size).await? {
            match e {
                ...
                Element {
                    ...
                    info.timecode_scale = Some(scale);
                }
                Element {
                    id: ids::DURATION,
